...
// Custom data configurable
        ...
        void ProcessIniConfig()
        {
            ...
            {
                enableAutoDoorCloser = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_ENABLE_AUTO_DOORS)
                    .ToBoolean(enableAutoDoorCloser);
                enableAirlockSystem = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_ENABLE_AIRLOCK)
                    .ToBoolean(enableAirlockSystem);
                ignoreAllHangarDoors = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_IGNORE_ALL_HANGAR_DOORS)
                    .ToBoolean(ignoreAllHangarDoors);
                regularDoorOpenDuration = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_REGULAR_DOOR_OPEN_TIME)
                    .ToDouble(regularDoorOpenDuration);
                hangarDoorOpenDuration = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_HANGAR_DOOR_OPEN_TIME)
                    .ToDouble(hangarDoorOpenDuration);
                doorExcludeString = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_DOOR_EXCLUDE_NAME)
                    .ToString(doorExcludeString);
                airlockInteriorDoorNameTag = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_INTERIOR_DOOR_NAME)
                    .ToString(airlockInteriorDoorNameTag);
                airlockExteriorDoorNameTag = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_EXTERIOR_DOOR_NAME)
                    .ToString(airlockExteriorDoorNameTag);
                airlockLightNameTag =
                    _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_LIGHT_NAME).ToString(airlockLightNameTag);
                airlockSoundNameTag =
                    _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_SOUND_NAME).ToString(airlockSoundNameTag);
                drawTitleScreen = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_DRAW_TITLE_SCREEN)
                    .ToBoolean(drawTitleScreen);
                AutoCloseOnlyFullyOpen = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_AUTOCLOSE_FULLY_OPEN)
                    .ToBoolean(AutoCloseOnlyFullyOpen);
            }
        ...
        void PrintDetailedInfo()
        {
            ...
            _detailedInfo.Append(
                $"Next refresh in {Math.Round(Math.Max(_scheduledGrabBlocks.RunInterval - _scheduledGrabBlocks.TimeSinceLastRun, 0))} seconds\n\n");
        ...
        void GrabBlocks()
        {
            ...
            for (int i = autoDoors.Count - 1; i >= 0; --i)
            {
                ...
                {
                    autoDoors[i].UpdateSettings(door is IMyAirtightHangarDoor
                        ? hangarDoorOpenDuration
                        : regularDoorOpenDuration);
                }
            }
            ...
            foreach (var block in allBlocks)
            {
                if (block is IMyDoor)
                {
                    ...
                    if (ShouldAddAutoDoor(block))
                    {
                        ...
                        {
                            double autoCloseInterval = door is IMyAirtightHangarDoor
                                ? hangarDoorOpenDuration
                                : regularDoorOpenDuration;
            ...
            foreach (var thisDoor in airlockDoors)
            {
                ...
                if (StringExtensions.Contains(nameLowercased,
                        airlockInteriorDoorNameTag)) //lists all airlockDoors with proper name 
                {
                    // Remove airlock tag
                    string thisName = nameLowercased.Replace(airlockInteriorDoorNameTag.ToLowerInvariant(), "");

                    // Remove exclude string
                    thisName = thisName.Replace($"[{doorExcludeString.ToLowerInvariant()}]", "")
                        .Replace(doorExcludeString.ToLowerInvariant(), ""); //remove door exclusion string 
            ...
            foreach (var hashValue in airlockNames)
            {
                ...
                foreach (var airlock in airlockList)
                {
                    ...
                    {
                        airlock.GetBlocks(hashValue, airlockDoors, allLights, allSounds, airlockInteriorDoorNameTag,
                            airlockExteriorDoorNameTag);
                        dupe = true;
                        break;
                    }
                }
                ...
                if (!dupe)
                    airlockList.Add(new Airlock(hashValue, airlockDoors, allLights, allSounds,
        ...
        public class AutoDoor
        {
            ...
            readonly string INI_COMMENT_DOOR_CUSTOM_AUTO_CLOSE_TIME =
                $" To use a custom auto close time, set \"{INI_DOOR_USE_DEFAULT_AUTO_CLOSE}\" to false";
            ...
            void ParseIni()
            {
                ...
                if (_ini.TryParse(Door.CustomData))
                {
                    ...
                    customAutoCloseTime = _ini.Get(INI_SECTION_DOOR, INI_DOOR_CUSTOM_AUTO_CLOSE_TIME)
                        .ToDouble(customAutoCloseTime);
                }
                ...
                _ini.SetComment(INI_SECTION_DOOR, INI_DOOR_CUSTOM_AUTO_CLOSE_TIME,
                    INI_COMMENT_DOOR_CUSTOM_AUTO_CLOSE_TIME);
            ...
            public void Update(double time)
            {
                ...
                float threshold = _wasOpen ? 0.999f : 0.001f;
        ...
        public class Airlock
        {
            ...
            public Airlock(string airlockName, List<IMyDoor> airlockDoors, List<IMyLightingBlock> allLights,
                List<IMySoundBlock> allSounds, string airlockInteriorDoorNameTag, string airlockExteriorDoorNameTag)
            {
                Name = airlockName;

                GetBlocks(this.Name, airlockDoors, allLights, allSounds, airlockInteriorDoorNameTag,
                    airlockExteriorDoorNameTag);
                Info =
                    $"    Interior Doors: {_airlockInteriorList.Count}\n    Exterior Doors: {_airlockExteriorList.Count}\n    Lights: {_airlockLightList.Count}\n    Sound Blocks: {_airlockSoundList.Count}";
            }
            ...
            void ProcessLightBlock(IMyLightingBlock l)
            {
                ...
                {
                    light.OpenLightEnabled = _ini.Get(INI_SECTION_LIGHT, INI_LIGHT_OPEN_ENABLE)
                        .ToBoolean(light.OpenLightEnabled);
                    light.OpenColor =
                        MyIniHelper.GetColor(INI_SECTION_LIGHT, INI_LIGHT_OPEN_COLOR, _ini, light.OpenColor);
                    light.OpenBlinkInterval = _ini.Get(INI_SECTION_LIGHT, INI_LIGHT_OPEN_INTERVAL)
                        .ToSingle(light.OpenBlinkInterval);
                    light.OpenBlinkLength = _ini.Get(INI_SECTION_LIGHT, INI_LIGHT_OPEN_LENGTH)
                        .ToSingle(light.OpenBlinkLength);
                    light.ClosedLightEnabled = _ini.Get(INI_SECTION_LIGHT, INI_LIGHT_CLOSED_ENABLE)
                        .ToBoolean(light.ClosedLightEnabled);
                    light.ClosedColor = MyIniHelper.GetColor(INI_SECTION_LIGHT, INI_LIGHT_CLOSED_COLOR, _ini,
                        light.ClosedColor);
                    light.ClosedBlinkInterval = _ini.Get(INI_SECTION_LIGHT, INI_LIGHT_CLOSED_INTERVAL)
                        .ToSingle(light.ClosedBlinkInterval);
                    light.ClosedBlinkLength = _ini.Get(INI_SECTION_LIGHT, INI_LIGHT_CLOSED_LENGTH)
                        .ToSingle(light.ClosedBlinkLength);
                }
            ...
            public void GetBlocks(string airlockName, List<IMyDoor> airlockDoors, List<IMyLightingBlock> allLights,
                List<IMySoundBlock> allSounds, string airlockInteriorDoorNameTag, string airlockExteriorDoorNameTag)
            {
        ...
        public class RuntimeTracker
        {
            ...
            public void AddRuntime()
            {
                ...
                int roundedTicksSinceLastRuntime =
                    (int)Math.Round(_program.Runtime.TimeSinceLastRun.TotalMilliseconds / MS_PER_TICK);
        ...
        public static class MyIniHelper
        {
                ...
                bool writeAlpha = true)
            {
                if (writeAlpha)
                {
                    ini.Set(sectionName, itemName,
                        string.Format("{0}, {1}, {2}, {3}", color.R, color.G, color.B, color.A));
                }
                else
                {
        ...
        public class ScheduledAction
        {
            ...
            public ScheduledAction(Action action, double runFrequency = 0, bool removeAfterRun = false,
                double timeOffset = 0)
            {
                _action = action;
        ...
        class AutoDoorScreenManager
        {
            ...
            float[] _openRatios = new float[]
            {
                1f, 0.83f, 0.67f, 0.50f, 0.33f, 0.16f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.1f, 0.16f, 0.33f, 0.50f, 0.67f,
                0.83f, 1f
            };
            ...
            void DrawTitleBar(IMyTextSurface _surface, MySpriteDrawFrame frame, float scale)
            {
                ...
                Vector2 titleBarTextPos = topLeft + new Vector2(_surface.TextureSize.X * 0.5f,
                    0.5f * (titleBarHeight - scale * BaseTextHeightPx));
            ...
            {
                frame.Add(new MySprite(Texture, "SquareSimple", new Vector2(50f, 17f) * scale + centerPos,
                    new Vector2(95f * doorOpenRatio, 165f) * scale, _white, null, Center, 0f)); // door right
                frame.Add(new MySprite(Texture, "SquareSimple", new Vector2(-50f, 17f) * scale + centerPos,
                    new Vector2(95f * doorOpenRatio, 165f) * scale, _white, null, Center, 0f)); // door left
                frame.Add(new MySprite(Texture, "RightTriangle", new Vector2(-37f, -52f) * scale + centerPos,
                    new Vector2(40f, 40f) * scale, _black, null, Center, 1.5708f)); // door left cornerCopy
                frame.Add(new MySprite(Texture, "RightTriangle", new Vector2(37f, -52f) * scale + centerPos,
                    new Vector2(40f, 40f) * scale, _black, null, Center, -3.1416f)); // door right cornerCopy
                frame.Add(new MySprite(Texture, "SquareSimple", new Vector2(80f, 0f) * scale + centerPos,
                    new Vector2(60f, 210f) * scale, _black, null, Center, 0f)); // door frame right outline
                frame.Add(new MySprite(Texture, "SquareSimple", new Vector2(-80f, 0f) * scale + centerPos,
                    new Vector2(60f, 210f) * scale, _black, null, Center, 0f)); // door frame left outline
                frame.Add(new MySprite(Texture, "SquareSimple", new Vector2(75f, 0f) * scale + centerPos,
                    new Vector2(40f, 200f) * scale, _white, null, Center, 0f)); // door frame right
                frame.Add(new MySprite(Texture, "SquareSimple", new Vector2(-75f, 0f) * scale + centerPos,
                    new Vector2(40f, 200f) * scale, _white, null, Center, 0f)); // door frame left
                frame.Add(new MySprite(Texture, "SquareSimple", new Vector2(0f, -85f) * scale + centerPos,
                    new Vector2(110f, 30f) * scale, _white, null, Center, 0f)); // door frame top
                frame.Add(new MySprite(Texture, "RightTriangle", new Vector2(40f, -55f) * scale + centerPos,
                    new Vector2(30f, 30f) * scale, _white, null, Center, -3.1416f)); // door right corner
                frame.Add(new MySprite(Texture, "RightTriangle", new Vector2(-40f, -55f) * scale + centerPos,
                    new Vector2(30f, 30f) * scale, _white, null, Center, 1.5708f)); // door left corner
            }
