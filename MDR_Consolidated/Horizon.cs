...
*/
        ...
        {
            get { return _taggedControllers.Count > 0 ? _taggedControllers : _allControllers; }
        }
        ...
        const string INI_GENERAL_ALT_TRANS = "Surface to Sealevel transition alt. (m)"; // Add comment
        ...
        readonly RunningSymbol _runningSymbol =
            new RunningSymbol(new string[] { "", ".", "..", "...", "....", "...", "..", "." });
        ...
        Horizon()
        {
            ...
            _scheduler = new Scheduler(new Program());
        ...
        void ParseIni()
        {
            ...
            if (_ini.TryParse(Me.CustomData))
            {
                // General
                _textSurfaceNameTag =
                    _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_TEXT_NAME).ToString(_textSurfaceNameTag);
                _timeToCollisionThreshold = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_COLLISION_THRESH)
                    .ToDouble(_timeToCollisionThreshold);
                _soundBlockNameTag = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_SOUND_NAME).ToString(_soundBlockNameTag);
                _collisionSoundInterval = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_SOUND_INTERVAL)
                    .ToDouble(_collisionSoundInterval);
                _referenceNameTag = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_REF_NAME).ToString(_referenceNameTag);
                _altitudeTransitionThreshold = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_ALT_TRANS)
                    .ToDouble(_altitudeTransitionThreshold);
                _showXYZAxis = _ini.Get(INI_SECTION_GENERAL, INI_GENERAL_SHOW_XYZ).ToBoolean(_showXYZAxis);
                _sunRotationAxis = MyIniHelper.GetVector3D(INI_SECTION_GENERAL, INI_GENERAL_SUN_ROTATION, _ini,
                    _sunRotationAxis);
                ...
                _spaceBackgroundColor =
                    MyIniHelper.GetColor(INI_SECTION_COLORS, INI_COLORS_SPACE, _ini, _spaceBackgroundColor);
                _progradeColor = MyIniHelper.GetColor(INI_SECTION_COLORS, INI_COLORS_PROGRADE, _ini, _progradeColor);
                _retrogradeColor =
                    MyIniHelper.GetColor(INI_SECTION_COLORS, INI_COLORS_RETROGRADE, _ini, _retrogradeColor);
                _textColor = MyIniHelper.GetColor(INI_SECTION_COLORS, INI_COLORS_TEXT, _ini, _textColor);
                _textBoxColor =
                    MyIniHelper.GetColor(INI_SECTION_COLORS, INI_COLORS_TEXT_BOX_OUTLINE, _ini, _textBoxColor);
                _textBoxBackground = MyIniHelper.GetColor(INI_SECTION_COLORS, INI_COLORS_TEXT_BOX_BACKGROUND, _ini,
                    _textBoxBackground);
                _horizonLineColor =
                    MyIniHelper.GetColor(INI_SECTION_COLORS, INI_COLORS_HORIZON_LINE, _ini, _horizonLineColor);
                _elevationLineColor = MyIniHelper.GetColor(INI_SECTION_COLORS, INI_COLORS_ELEVATION_LINE, _ini,
                    _elevationLineColor);
                _orientationLineColor = MyIniHelper.GetColor(INI_SECTION_COLORS, INI_COLORS_ORENTATION, _ini,
                    _orientationLineColor);
        ...
        void WriteIni()
        {
            ...
            _ini.SetComment(INI_SECTION_GENERAL, INI_GENERAL_COLLISION_THRESH,
        ...
        class ArtificialHorizon
        {
            ...
            double Altitude
            {
                get { return _surfaceAltitude >= _altitudeTransitionThreshold ? _sealevelAltitude : _surfaceAltitude; }
            }

            string AltitudeLabel
            {
                get { return _surfaceAltitude >= _altitudeTransitionThreshold ? "Sea level" : "Surface"; }
            }
            ...
            public void CalculateParameters(IMyShipController controller, double updatesPerSecond)
            {
                ...
                if (!Vector3D.IsZero(_velocity, 1e-2))
                {
                    ...
                    _flattenedVelocity.X =
                        (float)Math.Asin(MathHelper.Clamp(localVelocity.X, -1, 1)) * ONE_OVER_HALF_PI;
                    _flattenedVelocity.Y =
                        (float)Math.Asin(MathHelper.Clamp(-localVelocity.Y, -1, 1)) * ONE_OVER_HALF_PI;
            ...
            void CalculateArtificialHorizonParameters(IMyShipController controller, double updatesPerSecond)
            {
                ...
                _roll = (float)VectorMath.AngleBetween(flattenedUpVector, Vector3D.Up) *
                        Math.Sign(Vector3D.Dot(Vector3D.Right, flattenedUpVector));
                _pitch = (float)VectorMath.AngleBetween(forward, controller.WorldMatrix.Forward) *
                         Math.Sign(Vector3D.Dot(up, controller.WorldMatrix.Forward));
            ...
            void CalculateSpaceParameters(IMyShipController controller)
            {
                ...
                _axisDrawOrder[1] = (byte)MathHelper.Clamp((MyFixedPoint)(byte)(ALL_AXIS_ENUMS & ~usedAxes), 0, (byte)ALL_AXIS_ENUMS);
            }
            ...
            public void Draw(IMyTextSurface surface, bool clearSpriteCache)
            {
                ...
                using (var frame = surface.DrawFrame())
                {
                    ...
                    if (_inGravity)
                    {
                        ...
                        DrawTextBoxes(frame, surface, screenCenter, avgViewportSize, scale, $"{_speed:n1}",
                            $"{Altitude:0}", $"{_bearing:0}°");
                    ...
                    DrawLine(frame, new Vector2(0, screenCenter.Y),
                        new Vector2(screenCenter.X - 64 * scale, screenCenter.Y), HORIZON_THICKNESS * scale,
                        _orientationColor);
                    DrawLine(frame, new Vector2(screenCenter.X + 64 * scale, screenCenter.Y),
                        new Vector2(screenCenter.X * 2f, screenCenter.Y), HORIZON_THICKNESS * scale, _orientationColor);

                    Vector2 scaledIconSize = VELOCITY_INDICATOR_SIZE * scale;
                    MySprite centerSprite = new MySprite(SpriteType.TEXTURE, "AH_BoreSight",
                        size: scaledIconSize * 1.2f, position: screenCenter + Vector2.UnitY * scaledIconSize * 0.5f,
                        color: _orientationColor);
                    centerSprite.RotationOrScale = -MathHelper.PiOver2;
                    frame.Add(centerSprite);

                    // Draw velocity indicator
                    MySprite velocitySprite = new MySprite(SpriteType.TEXTURE, "AH_VelocityVector",
                        size: scaledIconSize, color: !_movingBackwards ? _progradeColor : _retrogradeColor);
                    ...
                    if (_movingBackwards)
                    {
                        ...
                        MySprite retrograteSprite = new MySprite(SpriteType.TEXTURE, "SquareSimple",
                            size: retrogradeCrossSize, color: _retrogradeColor);
            ...
            void DrawTextBoxes(MySpriteDrawFrame frame, IMyTextSurface surface, Vector2 screenCenter,
                Vector2 screenSize, float scale, string leftText, string rightText, string topText = "")
            {
                Vector2 boxSize = TEXT_BOX_SIZE * scale;
                float textSize = STATUS_TEXT_SIZE * scale;
                Vector2 leftBoxPos =
                    screenCenter +
                    new Vector2(-0.5f * (screenSize.X - boxSize.X),
                        boxSize.Y * 0.5f); //+ new Vector2(screenSize.X * -0.40f, boxSize.Y * 0.5f);
                Vector2 rightBoxPos =
                    screenCenter +
                    new Vector2(0.5f * (screenSize.X - boxSize.X),
                        boxSize.Y * 0.5f); //+ new Vector2(screenSize.X * 0.40f, boxSize.Y * 0.5f);
                ...
                DrawTextBox(frame, surface, boxSize, leftBoxPos, _textColor, _textBoxBorderColor,
                    _textBoxBackgroundColor, textSize, leftText, leftTitle);
                DrawTextBox(frame, surface, boxSize, rightBoxPos, _textColor, _textBoxBorderColor,
                    _textBoxBackgroundColor, textSize, rightText, rightTitle);

                if (_inGravity)
                {
                    MySprite altMode = MySprite.CreateText(AltitudeLabel, "Debug", _textColor, textSize * 0.75f,
                        TextAlignment.CENTER);
                    altMode.Position = screenCenter + new Vector2(0.5f * (screenSize.X - boxSize.X), boxSize.Y * 1.0f);
                    frame.Add(altMode);

                    MySprite verticalSpeedLabel = MySprite.CreateText(VERTICAL_SPEED, "Debug", _textColor,
                        textSize * 0.75f, TextAlignment.CENTER);
                    verticalSpeedLabel.Position =
                        screenCenter + new Vector2(-0.5f * (screenSize.X - boxSize.X), boxSize.Y * 1.0f);
                    frame.Add(verticalSpeedLabel);

                    MySprite verticalSpeed = MySprite.CreateText($"{_verticalSpeed:n1}", "Debug", _textColor,
                        textSize * 0.75f, TextAlignment.CENTER);
                    verticalSpeed.Position =
                        screenCenter + new Vector2(-0.5f * (screenSize.X - boxSize.X), boxSize.Y * 1.5f);
                ...
                if (!string.IsNullOrWhiteSpace(topText))
                {
                    ...
                    DrawTextBox(frame, surface, boxSize, topBoxPos, _textColor, _textBoxBorderColor,
                        _textBoxBackgroundColor, textSize, topText); //, drawBackground: false);
                }
            }

            void DrawTextBox(MySpriteDrawFrame frame, IMyTextSurface surface, Vector2 size, Vector2 position,
                Color textColor, Color borderColor, Color backgroundColor, float textSize, string text,
                string title = "")
            {
                ...
                MySprite background =
                    new MySprite(SpriteType.TEXTURE, "SquareSimple", color: backgroundColor, size: size);
            ...
            void DrawArtificialHorizon(MySpriteDrawFrame frame, Vector2 screenCenter, float scale, float minSideLength)
            {
                ...
                MySprite skySprite = new MySprite(SpriteType.TEXTURE, "SquareSimple", color: _skyColor,
                    size: skySpriteSize);
                skySprite.RotationOrScale = _roll;

                Vector2 skyMidPt =
                    screenCenter + new Vector2(0, -skySpriteSize.Y * 0.5f); //surfaceSize.Y * new Vector2(0.5f, -1f);
                skySprite.Position = skyMidPt + _rollOffset + _pitchOffset * pitchProportion;
                frame.Add(skySprite);

                // Draw horizon line
                MySprite horizonLineSprite = new MySprite(SpriteType.TEXTURE, "SquareSimple", color: _horizonLineColor,
                    size: new Vector2(skySpriteSize.X, HORIZON_THICKNESS * scale));
            ...
            void DrawAltitudeWarning(MySpriteDrawFrame frame, Vector2 screenCenter, Vector2 screenSize, float scale,
                IMyTextSurface surface)
            {
                if (CollisionWarning)
                {
                    if (_showPullUp)
                    {
                        ...
                        Vector2 textBoxSize = surface.MeasureStringInPixels(_pullUpStringBuilder, "Debug", textSize) +
                                              scale * 24f;
                        Vector2 textPosition = screenCenter + new Vector2(0, screenSize.Y * 0.25f);
                        DrawTextBox(frame, surface, textBoxSize, textPosition, _altitudeWarningColor,
                            _altitudeWarningColor, _textBoxBackgroundColor, textSize, PULL_UP_TEXT);
                    }
                    ...
                    MySprite warningCrossHalf = MySprite.CreateSprite("AH_BoreSight",
                        screenCenter + warningCrossPosition, warningCrossSize);
            ...
            void DrawElevationLadder(MySpriteDrawFrame frame, Vector2 midPoint, Vector2 size, float basePitchProportion,
                float elevationAngleDeg, float scale, Color color, bool drawText)
            {
                float pitchProportion = MathHelper.ToRadians(-elevationAngleDeg) / MathHelper.PiOver2;
                string textureName = pitchProportion <= 0
                    ? "AH_GravityHudPositiveDegrees"
                    : "AH_GravityHudNegativeDegrees";
                Vector2 scaledSize = size * scale;

                MySprite ladderSprite = new MySprite(SpriteType.TEXTURE, textureName, color: _elevationLineColor,
                    size: scaledSize);
            ...
            void DrawSpace(MySpriteDrawFrame frame, Vector2 screenCenter, float halfExtent, float scale)
            {
                ...
                foreach (var axis in _axisDrawOrder)
                {
                    ...
                    {
                        DrawArrowHead(frame, xPos, AXIS_MARKER_SIZE * scale, _xAxisDirn, _axisZCosVector.X, _xAxisColor,
                            _axisArrowBackColor);
                        frame.Add(xLine);
                        frame.Add(xLabel);
                    }
                    ...
                    {
                        DrawArrowHead(frame, yPos, AXIS_MARKER_SIZE * scale, _yAxisDirn, _axisZCosVector.Y, _yAxisColor,
                            _axisArrowBackColor);
                        frame.Add(yLine);
                        frame.Add(yLabel);
                    }
                    ...
                    {
                        DrawArrowHead(frame, zPos, AXIS_MARKER_SIZE * scale, _zAxisDirn, _axisZCosVector.Z, _zAxisColor,
                            _axisArrowBackColor);
                        frame.Add(zLine);
                        frame.Add(zLabel);
                    }
            ...
            void DrawArrowHead(MySpriteDrawFrame frame, Vector2 position, Vector2 arrowSize, Vector2 flattenedDirection,
        ...
        public class Scheduler
        {
                ...
                double timeOffset = 0)
            {
                ScheduledAction scheduledAction =
                    new ScheduledAction(action, updateFrequency, disposeAfterRun, timeOffset);
